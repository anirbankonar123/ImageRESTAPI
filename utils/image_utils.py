...
def write_preprocessed_img(input_format,input):
    ...
    gridSize=0
    iteration_erosion=0
    kernelSize_erosion=0
    iteration_dilation=0
    kernelSize_dilation=0
    img_resized_cv2 = ""
    brightNessFactor = 0
    sharpnessFactor = 0
    ...
    for filter in input.filters:
        for filterParam in filter.filterParams:
            paramKey=filterParam.paramKey
            paramValue=filterParam.paramValue

            if filter.filterName == "resize":
                if paramKey=="width":
                    target_w = int(paramValue)

            if filter.filterName == "adaptiveHistEq":
                if paramKey=="clipLimit":
                    clipLimit = float(paramValue)
                if paramKey=="gridSize":
                    gridSize=(int(paramValue),int(paramValue))

          
        if filter.filterName == "resize":
            print("applying resize")
            target_ht = np.round((asp_ratio * target_w), 0).astype(int)
            if (loop_ctr == 0):
                im_pil = Image.fromarray(img)
            else:
                im_pil = Image.fromarray(image)

            img_resized = resize_image(im_pil, w, h, target_w, target_ht)
            image = np.asarray(img_resized)

        if filter.filterName == "grayScale":
            grayScaleFlg = True
    ...
    if (resizeFlg):
        if (input_format == "png"):
            cv2.imwrite("tmp_resized.png", img_resized_cv2)
        elif (input_format == "jpg"):
            cv2.imwrite("tmp_resized.jpg", img_resized_cv2)
